    ...
    def parse(
        ...
        *,
        ...
        preview = ImportPreview(
            ...
            roster,
            tuple(
                ReviewRow(str(uuid5(UUID(import_id), f"schedule:{r.source_row}")), r)
                for r in parsed.review_rows
            ),
            source,
            (),
            config,
...
def issue(code: str, row=None, field=None, *, warning=False) -> ReviewIssue:
    ...
    return ReviewIssue(
        ...
        field,
        not warning,
        "Correct the value or explicitly exclude the source row." if row else None,
    )
...
def to_day(preview: ImportPreview) -> OperationalDay:
    return OperationalDay(
        ...
        preview.employees,
        tuple(
            EmployeeShift(r.employee_id, r.start, r.end, r.normalized_role)
            for wrapped in preview.rows
            if not (r := wrapped.values).excluded
            and not r.vacancy
...
def revalidate(preview: ImportPreview) -> ImportPreview:
    ...
    issues = [
        i
        for i in preview.source_issues
        if i.severity == IssueSeverity.FATAL
    ...
    for wrapped in preview.rows:
        ...
        if not row.vacancy and row.employee_id not in employee_ids:
            code = (
                row.match_status
                if row.match_status in {"UNMATCHED_EMPLOYEE", "AMBIGUOUS_EMPLOYEE"}
                else "UNMATCHED_EMPLOYEE"
            )
        ...
        if row.start is None or row.end is None:
            ...
            if (row.start.utcoffset() is None) != (row.end.utcoffset() is None):
                ...
                row.end - row.start
            ).total_seconds() > preview.import_config.maximum_shift_hours * 3600:
                issues.append(issue("IMPLAUSIBLY_LONG_SHIFT", row.source_row, "end"))
            if row.imported_hours is not None and (row.start.utcoffset() is None) == (
                row.end.utcoffset() is None
            ):
                if (
                    abs((row.end - row.start).total_seconds() / 3600 - row.imported_hours)
                    > preview.import_config.hours_tolerance
                ):
                    issues.append(
                        issue("HOURS_DURATION_MISMATCH", row.source_row, "hours", warning=True)
                    )
        if not row.vacancy and row.normalized_role == OperationalRole.UNKNOWN:
            issues.append(
                issue("UNKNOWN_POSITION", row.source_row, "normalized_role", warning=True)
            )
    day = to_day(preview)
    active_rows = [
        r.values
        for r in preview.rows
        if not r.values.excluded
        and not r.values.vacancy
        and r.values.employee_id is not None
        and r.values.start is not None
        and r.values.end is not None
    ]
    for item in validate_config(preview.config) + validate_operational_day(day, preview.config):
        ...
        issues.append(
            ReviewIssue(item.code, IssueSeverity.ERROR, item.message, source_row, item.path)
        )
    return replace(preview, issues=tuple(issues))


def apply_corrections(
    preview: ImportPreview, corrections: tuple[RowCorrection, ...]
) -> ImportPreview:
    rows = {r.row_id: r for r in preview.rows}
    employees = {e.employee_id: e for e in preview.employees}
    seen = set()
    employee_changes: dict[tuple[str, str], object] = {}
    errors = []
    for correction in corrections:
        ...
        row = wrapped.values
        if any(
            getattr(correction, field) is not None
            and not isinstance(getattr(correction, field), datetime)
        ...
        if correction.normalized_role is not None and not isinstance(
            correction.normalized_role, OperationalRole
        ):
        ...
        changes["required_fields_missing"] = tuple(
            f for f in row.required_fields_missing if f not in resolved
        )
        ...
        if correction.enabled is not None or correction.qualifications is not None:
            ...
            overrides = {
                ...
                for name in ("enabled", "qualifications")
                if getattr(correction, name) is not None
            }
            if ("enabled" in overrides and type(overrides["enabled"]) is not bool) or (
                "qualifications" in overrides
                and (
                    not isinstance(overrides["qualifications"], frozenset)
    ...
    return revalidate(
        replace(
            ...
            employees=tuple(employees[e.employee_id] for e in preview.employees),
