...
def test_persistent_api_reproducibility_and_lists(client: TestClient) -> None:
    ...
    listing = client.get(
        ...
        params={"operational_date": payload["operational_day"]["operational_date"]},
    )
    ...
    completed_result(client, stored)
    job = client.get(f"/api/v1/optimization-jobs/{stored.json()['id']}").json()
    run = client.get(f"/api/v1/optimization-runs/{job['result_run_id']}").json()
    assert run["input_hash"] == day["input_hash"]
    assert _without_runtime(run["result"]) == _without_runtime(direct_result)
...
@pytest.mark.parametrize(
    "url", ["/api/v1/operational-days/not-a-uuid", "/api/v1/optimization-runs/not-a-uuid"]
)
