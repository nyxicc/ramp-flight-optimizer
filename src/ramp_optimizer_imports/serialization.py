...
from ramp_optimizer.models import Employee, Flight, ScheduleReviewRow
...
def load_preview(serialized: str) -> ImportPreview:
    ...
    def employee(item):
        return Employee(
            **{
                **item,
                "qualifications": frozenset(Qualification(q) for q in item["qualifications"]),
            }
        )
    ...
    def flight_row(item):
        flight = dict(item["flight"])
        for field in ("arrival_time", "departure_time"):
            flight[field] = datetime.fromisoformat(flight[field]) if flight[field] else None
        return FlightReviewRow(
            **{
                **item,
                "flight": Flight(**flight),
                "status": FlightStatus(item["status"]),
                **{
                    field: tuple(item[field])
                    for field in ("unresolved_fields", "formula_fields", "normalized_fields")
                },
            }
        )

    policy = value.get("flight_policy")
    settings = value["import_config"]
    return ImportPreview(
        revision=value["revision"],
        operational_date=date.fromisoformat(value["operational_date"]),
        roster=tuple(employee(e) for e in value["roster"]),
        employees=tuple(employee(e) for e in value["employees"]),
        rows=tuple(ReviewRow(r["row_id"], parsed_row(r["values"])) for r in value["rows"]),
        source_issues=tuple(issue(i) for i in value["source_issues"]),
        issues=tuple(issue(i) for i in value["issues"]),
        config=OptimizerConfig(**value["config"]),
        import_config=TeamWorkImportConfig(
            **{
                **settings,
                "position_role_mappings": tuple(
                    (label, OperationalRole(role))
                    for label, role in settings["position_role_mappings"]
                ),
                "vacancy_position_placeholders": frozenset(
                    settings["vacancy_position_placeholders"]
                ),
            }
        ),
        corrections=tuple(correction(c) for c in value["corrections"]),
        flight_rows=tuple(flight_row(r) for r in value.get("flight_rows", ())),
        flight_policy=FlightTimePolicy(
            **{
                **policy,
                "operational_day_start": time.fromisoformat(policy["operational_day_start"]),
            }
        )
        if policy
        else None,
        detected_date=date.fromisoformat(value["detected_date"])
        if value.get("detected_date")
        else None,
        operational_date_correction=(
            date.fromisoformat(value["operational_date_correction"][0]),
            date.fromisoformat(value["operational_date_correction"][1]),
        )
        if value.get("operational_date_correction")
        else None,
        flight_corrections=tuple(
            FlightCorrection(
                c["row_id"],
                tuple(tuple(v) for v in c["changes"]),
                tuple(tuple(v) for v in c["original_values"]),
            )
            for c in value.get("flight_corrections", ())
        ),
    )
...
def preview_counts(preview):
    blocking_rows = {
        i.source_row for i in preview.issues if i.blocks_confirmation and i.source_row is not None
    }
    return {
        ...
        "accepted_shift_count": sum(
            not r.values.excluded
            and not r.values.vacancy
            and r.values.source_row not in blocking_rows
